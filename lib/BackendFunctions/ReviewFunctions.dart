...
class ReviewFunctions {
  static Future<bool> saveReview(Review review) async {
    ...
    int res = await Store.supabase.rpc('savereview', params: {
      'spot': review.spot!.id,
      'liked': false,
    ...
    if (res != 0) {
      ...
      final storageResponse = await Store.supabase.storage
          .from('reviewpics/reviewpics/public/')
          .upload(res.toString(), file);
      if (Store.snackbarKey.currentContext != null) {
        UITemplates.showErrorMessage(Store.snackbarKey.currentContext!,
  ...
  static Future<List<Review>> getMyReviews() async {
    ...
    await Future.forEach(res, (element) async {
      Review review = Review.fromMap(element as Map<String, dynamic>,
          Spot("", 0, 0, "", "", SpotTypes.Spot));
      review.author = await RelationshipFunctions.getAuthorOfReview(review);
  ...
  static Future<bool> deleteReview(Review rev) async {
    //todo: return
    var res = await Store.supabase.rpc('deletereview', params: {
      'reviewid': rev.id,
  ...
  static Future<bool> reportReview(Review rev, String subject) async {
    var res = await Store.supabase.rpc('report', params: {
      'object': rev.id,
      'subject': subject
    }).onError((error, stackTrace) => false);
  ...
  static Future<void> incrementReaction(
      Reactions reaction, Review review) async {
    print("incre");
    var res = await Store.supabase.rpc('incrementreaction', params: {
      'review': review.id,
      'type': reaction.name
  ...
  static Future<void> decrementReaction(
      Reactions reaction, Review review) async {
    print("decre");
    var res = await Store.supabase.rpc('decrementreaction', params: {
      'review': review.id,
      'type': reaction.name
    }).onError((error, stackTrace) {
      print(stackTrace);
