...
class SpotFunctions {
  static Future<List<Spot>> getSpots(
      String long, String lat, double radius) async {
    ...
    if (str != "") {
      try {
        res = await Store.supabase
            .rpc('fulltextspotsearch', params: {"str": str});
      } catch (e) {}
    } else {
      return [];
    }
  ...
  static Future<List<RawSpot>> getallspotsnearyou(double lat, long) async {
    var res = await Store.supabase
        .rpc('getallspotsnearyou', params: {"lat": lat, "long": long});
  ...
  static Future<bool> saveSpot(Spot spot) async {
    int res = await Store.supabase.rpc('savespot', params: {
      'name': spot.name,
      'adress': spot.adress,
  ...
  static Future<Spot?> getspotfromid(String id) async {
    if (id != "") {
      var res = await Store.supabase
          .rpc('getspotfromid', params: {"id": int.tryParse(id)});
