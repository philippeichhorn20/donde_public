...
class SignUpFunctions {
  static Future<User?> signUp(
      String phone, password, username, uusername) async {
    ...
    if (user != null) {
      await Store.supabase.rpc('newuser2', params: {
        'userid': res.user!.id,
        'username': username,
  ...
  static Future<LogInState> logInFromStorage() async {
    try {
      await Supabase.initialize(
        ...
        anonKey:
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inpncmd0aWF0anJ5cnlvd3F3aGhpIiwicm9sZSI6ImFub24iLCJpYXQiOjE2NzkyMzgxNzAsImV4cCI6MTk5NDgxNDE3MH0.jZqek_ImEiQpkR8WJ-XD7yPoSxzC12aGIhH3NN46xh0",
      );
    } catch (e) {}
  ...
  static Future<bool> checkUniqueness(String uusername) async {
    var res = await Store.supabase.rpc('checkuniqueness', params: {
      'unique_username': uusername.toLowerCase(),
  ...
  static Future<bool> deleteUser() async {
    ...
    var res = await Store.supabase
        .rpc('deleteuser', params: {'user_id': Store.me.id});
    OneSignal.logout();
    return success;
  }
  ...
  static Future<bool> changeDetails(String uusername, name) async {
    bool success = true;
    print("changing");
    var res = await Store.supabase.rpc('changedetails', params: {
      'user_id': "",
      'unique_username': uusername,
      'username': name,
    });
...
enum LogInState { LOGGED_IN, LOGGED_OUT, NO_COONECTION }
