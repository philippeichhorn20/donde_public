...
class Linking {
  ...
  static Future<String> createLinkToSpot(Spot spot) async {
    final dynamicLinkParams = DynamicLinkParameters(
      link: Uri.parse(
          "https://opendonde.page.link/hey?user=${Store.me.id}&spot=${spot.id}"),
      uriPrefix: "https://opendonde.page.link",
      androidParameters:
          const AndroidParameters(packageName: "com.example.app.android"),
      iosParameters: const IOSParameters(bundleId: "com.example.app.ios"),
  ...
  static Future<void> handlePotentialLinks(GlobalKey snackbarKey) async {
    FirebaseDynamicLinks.instance.onLink
      ...
      if (data.link.queryParameters["spot"] != null) {
        spot = await SpotFunctions.getspotfromid(
            data.link.queryParameters["spot"]!);
      }
      if (user != null) {
        if (snackbarKey.currentContext != null) {
          ScaffoldMessenger.of(snackbarKey.currentContext!)
              .showSnackBar(ListTiles.showSnackbar(user));
        }
      }
      if (spot != null) {
        showDialog(
            ...
            return BackdropFilter(
              ...
              child: Padding(
                ...
                child: Container(
                    ...
                    child:
                        SingleChildScrollView(child: SpotView(spot!, () {}))),
              ),
            );
          },
        );
      ...
      if (data.link.queryParameters["spot"] != null) {
        Spot? spot = await SpotFunctions.getspotfromid(
            data.link.queryParameters["spot"]!);
      }
    });
  }
