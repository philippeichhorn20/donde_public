...
class GetFriends {
  static Future<List<MyUser>> getFriendsFromContact(String contact) async {
    ...
    FirebaseAnalytics.instance
        .logEvent(name: "uses_proposals", parameters: {"result": users.length});
  ...
  static Future<List<MyUser>> getPeopleThatFollowMe() async {
    ...
    FirebaseAnalytics.instance.logEvent(
        name: "searches_followers", parameters: {"result": users.length});
    users.removeWhere((element) => element.id == Store.me.id);
  ...
  static Future<List<MyUser>> getreactingusers(
      Review review, Reactions reaction) async {
    var res = await Store.supabase.rpc('getreactingusers',
        params: {"review": review.id, "reaction": reaction.name});
  ...
  static Future<List<MyUser>> getUserFromString(String str) async {
    ...
    var res = await Store.supabase.rpc('getuserfromstring',
        params: {"str": str}).onError((error, stackTrace) => {print(error)});
...
//deprecated
  static Future<List<MyUser>> getPeopleIFollow() async {
    ...
    FirebaseAnalytics.instance.logEvent(
        name: "searches_following", parameters: {"result": users.length});
    users.removeWhere((element) => element.id == Store.me.id);
