   ...
   */
        child: Column(
          ...
          children: [
                      ...
                      TextSpan(
                          ...
                          recognizer: TapGestureRecognizer()
                            ..onTap = () async {
                              ...
                              Share.share("Check out this spot: " +
                                  widget.spot.name +
                                  " at " +
                                  widget.spot.adress +
                                  "\nAdd me here: $link");
                          ...
                          recognizer: TapGestureRecognizer()
                            ..onTap = () {
                              if (widget.spot.latlong != null) {
                                launchUrl(Uri.parse(
                                    'https://www.google.com/maps/search/?api=1&query=${widget.spot.lat},${widget.spot.long}'));
                              } else {
                                launchUrl(Uri.parse(
                                    'https://www.google.com/maps/search/?api=1&query=${widget.spot.adress}'));
                              }
                            }),
                    ],
                  ),
            ...
            Padding(
              padding:
                  const EdgeInsets.only(left: 20, top: 0, bottom: 2, right: 20),
              child: Container(
                child: RichText(
                  text: TextSpan(
                    children: <TextSpan>[
                      ...
                      TextSpan(
                        text: "\t\t•\t\t" + widget.spot.getDistance(),
                        style: UITemplates.clickableText,
                      ),
                      TextSpan(
                        text:
                            "\t\t•\t\t ${ratingAvg} (${widget.spot.reviews.length})",
                        style: UITemplates.clickableText,
                      ),
                      TextSpan(
                        text: "\n" + widget.spot.adress,
                  ...
                  ? PageView.builder(
                      ...
                      itemBuilder: (context, index) {
                        if (widget.spot.reviews.length > index + 1) {
                          ReviewFunctions.getReviewPic(
                              widget.spot.reviews[index + 1]);
                        }
                        return Padding(
